...
//
//  Created by Le Dai Vuong on 12/26/18.
...
class AppFunctions {
    ...
    static func deployApp(appModule: App, apps: [PreviewApp]) {
        ...
        for id in appIds {
            this.waitForDeployAppSuccess(appModule: appModule, appId: id)
        }
    }
    ...
    static func createApp(appModule: App, appName: String = "Test App",  spaceId: Int? = nil, threadId: Int? = nil) -> Int {
        ...
        _ = waitForPromises(timeout: TestsConstants.WAIT_FOR_PROMISE_TIMEOUT)
        this.deployApp(appModule: appModule, apps: apps)
        return appId
    }
    
    static func createApps(appModule: App, appName: String = "Test App",  spaceId: Int? = nil, threadId: Int? = nil, amount: Int = 1) -> [Int] {
        var apps = [PreviewApp]()
        var appIds = [Int]()
    ...
    static func deleteApp(appId: Int) {
        this.devAppModule.deleteApp(appId)
            .then{res in
    ...
    static func generateToken(appModule: App, appId: Int) -> String {
        ...
        var apiToken: String!
        let getListAPIsTokenResponse = this.getListAPIsToken(appId)
        var tokens = [Token]()
        ...
        _ = waitForPromises(timeout: TestsConstants.WAIT_FOR_PROMISE_TIMEOUT)
        this.deployApp(appModule: appModule, apps: [PreviewApp(appId)])
        return apiToken
    }
    ...
    static func updateAppPermissions(appModule: App, appId: Int, right: Right) -> String {
        ...
        var revision: String!
        var rights = this.getAppPermissions(appId: appId)
        rights.append(right)
    ...
    static func updateMiscSetting(appModule: App, code: String, id: Int, name: String, decimalPrecision: Int = 16, decimalScale: Int = 4, enableBulkDeletion: Bool = false, fiscalYearStartMonth: Int = 4, roundingMode: String = "HALF_EVEN", useComment: Bool = true, useHistory: Bool = true, useThumbnail: Bool = true) {
        ...
        _ = waitForPromises(timeout: TestsConstants.WAIT_FOR_PROMISE_TIMEOUT)
        this.deployApp(appModule: appModule, apps: [PreviewApp(id)])
    }
}

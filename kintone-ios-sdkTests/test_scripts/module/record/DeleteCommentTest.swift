        ...
        describe("DeleteComment") {
            ...
            it("Test_256_Success_ValidData_GuestSpace") {
                let recordModuleGuestSpace = Record(TestCommonHandling.createConnection(
                    TestConstant.Connection.CRED_ADMIN_USERNAME, TestConstant.Connection.CRED_ADMIN_PASSWORD, guestSpaceId))
                let addData: Dictionary<String, FieldValue> = [:]
                ...
                let addCommentResponse = TestCommonHandling.awaitAsync(recordModuleGuestSpace.addComment(
                    guestSpaceAppId, recordGuestSpaceId, comment)) as! AddCommentResponse
                commentId = addCommentResponse.getId()
            ...
            it("Test_257_Error_InvalidCommentId") {
                let result = TestCommonHandling.awaitAsync(recordModule.deleteComment(appId, recordId, nonexistentId)) as! KintoneAPIException
            ...
            it("Test_261_Error_InvalidAppId") {
                let result = TestCommonHandling.awaitAsync(recordModule.deleteComment(nonexistentId, recordId, commentId)) as! KintoneAPIException
            ...
            it("Test_262_Error_InvalidRecordId") {
                let result = TestCommonHandling.awaitAsync(recordModule.deleteComment(appId, nonexistentId, commentId)) as! KintoneAPIException

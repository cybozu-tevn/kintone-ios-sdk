...
open class DevConnection: NSObject {
    ...
    public init(_ domain: String?, _ auth: DevAuth, _ guestSpaceID: Int) {
        self.domain = domain
        self.devAuth = auth
    ...
    ///   - domain: Kintone domain url
    ///   - auth: Credential information
    public convenience init(_ domain: String, _ auth: DevAuth) {
    ...
    open func downloadFile(_ baseUrl: String, _ body: String) -> Promise<Data> {
        return Promise { fullfill, reject in
            ...
            do {
                urlString = try self.getURL(baseUrl, ConnectionConstants.FILE, nil)
            } catch {
    ...
    open func request(_ baseUrl: String, _ method: String, _ apiName: String, _ body: String) -> Promise<Data> {
        return Promise { fullfill, reject in
            ...
            do {
                urlString = try self.getURL(baseUrl, apiName, nil)
            } catch {
                reject(KintoneAPIException("Invalid URL"))
            }
    ...
    /// - Returns: String
    private func getURL(_ baseUrl: String, _ apiName: String?, _ parameters: String?) throws -> String {
